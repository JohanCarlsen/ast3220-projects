...
# def element_abundance(lnT, variables, Omega_b0=None):
...
# initial_Boltzmann = [Yn_i, Yp_i, YD_i]
# sol = solve_ivp(Boltzmann, T_range, initial_Boltzmann, method='Radau', rtol=1e-12, atol=1e-12, dense_output=True)
...
# rel_error = np.abs(Yp + Yn + YD - 1) / (Yp + Yn + YD)

# ax.plot(T_plot, rel_error, lw=1, color='black', label=r'$\frac{|\sum Y_i-1|}{\sum Y_i}$')
# ax.invert_xaxis()
# ax.set_xscale('log')
# ax.set_yscale('log')
# ax.set_xlabel('T[K]')
...
# ax.legend()
# plt.savefig('figures/rel_err.pdf')
# plt.savefig('figures/rel_err.png')
...
# ax.plot(T_plot, Yn, color='royalblue', label=r'$n$')
# ax.plot(T_plot, Yn_equi, color='royalblue', ls='dotted')
# ax.plot(T_plot, 2 * YD, color='green', label=r'$D$')
# ax.set_xlabel('T [K]')
...
# ax.set_yscale('log')
# fig.tight_layout()
# # plt.savefig('figures/fractions_with_D.pdf')
# # plt.savefig('figures/fractions_with_D.png')
...
# sol = solve_ivp(element_abundance, T_range, initial_abundance, method='Radau', rtol=1e-12, atol=1e-12, dense_output=True)
...
# print(f'Total time to compute the mass fractions: {time//60:.0f} min {time%60:.0f} s.')
...
# fig, ax = plt.subplots()
# ax.plot(T_plot, Yp, label=r'p')
# ax.plot(T_plot, Yn, label=r'n')
# ax.plot(T_plot, 2 * YD, label=r'D')
# ax.plot(T_plot, 3 * YT, label=r'T')
...
# 	sol = solve_ivp(element_abundance, T_range, initial_abundance, args=(O_b0[i],), method='Radau', rtol=1e-12, atol=1e-12)
...
# print(f'\nComputed models for {n_runs} O_b0 values in: {time_outer//60:2.0f} min {time_outer%60:.0f} s')
...
# 	ax2.set_ylim([3e-11, 1e-3])
# 	ax2.set_xscale('log')
# 	ax2.set_yscale('log')
# 	ax2.set_ylabel(r'$Y_i/Y_p$')

# 	ax3.plot(O_b0, prob, lw=1, color='black')
# 	# ax3.set_xlim([1e-2, 1])
# 	ax3.set_ylim([0, 1.1])
...
# 	fig.savefig('figures/best-omega-b0.pdf')
# 	fig.savefig('figures/best-omega-b0.png')
