...
ax2.plot([best_Neff, best_Neff], [.7e-5, 4e-5], ls='dotted', lw=1, color='black')
ax2.set_ylabel(r'$Y_i/Y_p$')
ax2.set_ylim([.7e-5, 4e-5])
ax2.legend()

ax3.plot(Neff, YLi7Yp, color='red', label=r'Li$^7$')
ax3.fill_between(Neff, obs_YLi7Yp_lower, obs_YLi7Yp_upper, color='red', alpha=.4)
ax3.plot([best_Neff, best_Neff], [1e-10, 5e-10], ls='dotted', lw=1, color='black')
ax3.set_ylabel(r'$Y_i/Y_p$')
ax3.set_ylim([1e-10, 5e-10])
ax3.legend()

ax4.plot(Neff, prob/np.max(prob), color='black')
ax4.set_xlim([1, 5])
